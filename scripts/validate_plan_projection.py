...
def validate_plan_enhancement(errors: list[str]):
    rel = PLAN_VIEW.relative_to(ROOT)
    text = PLAN_VIEW.read_text(encoding="utf-8")
    legacy = (
        "route_endpoint",
        "flow.type==='spot'",
        "flow.type==='activity'",
    ...
    for token in legacy:
        if token in text:
            fail(errors, rel, "$enhancement", f"legacy Plan enhancement behavior remains: {token}")
