...
この配下は `_prototype/poc/README.md` に従う **non-canonical / non-runtime / non-production** の独立検証です。
...
有効性が確認できた場合は、このPoC構造をそのまま正式Schemaへ昇格させず、`leisure-concretize` / Concrete Plan Contract / View pipelineのownerで正式設計する。
正式実装で同等機能を確認したら本PoCは削除対象とする。
