...
const modeLabel = (mode='') => ({train:'電車',air:'飛行機',rental_car:'レンタカー',rental_car_and_mountain_access:'車＋登山アクセス',walk:'徒歩',bus:'バス'}[mode] || mode);
const activityLabel = (v='') => ({rental_car_pickup:'レンタカー受取',rental_car_return:'レンタカー返却',shopping_and_hiking_preparation:'買い出し・登山準備',optional_local_time:'自由時間・予備枠'}[v] || v.replaceAll('_',' '));
...
function endpointInline(endpoint) {
  ...
  if (endpoint.type === 'travel_point') {
    const kind = endpoint.point_type ? `<span class="badge subtle">${esc(endpoint.point_type)}</span>` : '';
...
function routeStops(item) {
  ...
  return `<div class="route-stops">${refs.map((x,i)=>`<span class="route-stop"><span class="stop-no">${i+1}</span>${refInline(x)}${x.role?` <small>${esc(x.role)}</small>`:''}</span>`).join('<span class="route-arrow">→</span>')}</div>`;
}
...
function flowItemHtml(item) {
  ...
  if (item.type === 'route') {
    const traversal = item.traversal_id && item.traversal_id !== 'canonical' ? `<span class="badge">${esc(item.direction || item.traversal_id)}</span>` : '';
...
function dayHtml(day) {
  ...
  return `<article class="day-card">
    <header class="day-head"><div class="day-number">Day ${esc(day.day)}</div><div><h3>${esc(day.purpose||'')}</h3>${day.appeal?`<p>${esc(day.appeal)}</p>`:''}</div></header>
...
function tripValueHtml(v={}) {
  ...
  return `<section class="section value-section"><h2>このPlanの価値</h2>${v.summary?`<p class="summary">${esc(v.summary)}</p>`:''}${highlights}${rows.length?`<dl class="fact-grid">${rows.map(([k,x])=>`<div><dt>${esc(k)}</dt><dd>${esc(x)}</dd></div>`).join('')}</dl>`:''}</section>`;
}
...
function renderHome(manifest) {
  ...
  app.innerHTML=`<section class="hero"><div class="kicker">PersonalOS Leisure</div><h1>レジャー</h1><p class="summary">Planを入口に、旅程 → Route → Spotへ掘り下げます。</p></section>${section('Plans','plan')}${section('Routes','route')}${section('Spots','spot')}`;
}

function renderPlan(data) {
  const p=data.plan||{},m=data.map||null; breadcrumb.innerHTML=`<a href="./">Home</a><span>›</span><span>${esc(data.id)}</span>`;
  app.innerHTML=`<section class="hero"><div class="kicker">Plan · ${esc(data.id)}</div><h1>${esc(data.title)}</h1><p class="summary">${esc(data.summary||'')}</p></section>
    ...
    <section class="section itinerary"><div class="section-heading"><div><h2>Dayごとの旅程</h2><p>魅力と、旅程を成立させる移動・宿泊・準備を同じFlowで表示します。</p></div></div>${(p.days||[]).map(dayHtml).join('')}</section>
...
function renderRoute(data) {
  ...
  app.innerHTML=`<section class="hero"><div class="kicker">Route · ${esc(data.id)}</div><h1>${esc(data.title)}</h1><p class="summary">${esc(data.summary||'')}</p><p>${esc(r.purpose||'')}</p></section>
    ${m?`<section class="section"><h2>Map</h2>${hasActual?`<div class="map-tabs"><button id="map-conceptual" class="map-tab active">計画</button><button id="map-actual" class="map-tab">実道路</button></div><div id="route-summary" class="route-summary" hidden><div><strong>${actualKm?`${actualKm} km`:''}</strong>${actualMinutes?`<span> 約${actualMinutes}分</span>`:''}</div></div>`:''}<div class="map-wrap"><div id="map"></div><div id="map-message" class="map-message"></div></div><p id="map-mode-note" class="note">${esc(m.note||'')}</p></section>`:''}
    ${(r.highlights||[]).length?`<section class="section"><h2>このRouteの魅力</h2><div class="cards">${r.highlights.map(x=>`<article class="card">${esc(x)}</article>`).join('')}</div></section>`:''}
    ${traversals.length?`<section class="section"><h2>順序・向き</h2>${traversals.map(t=>`<article class="card"><h3>${esc(t.direction||t.traversal_id)}</h3><div class="route-stops">${(t.ordered_spot_refs||[]).map((x,i)=>`<span class="route-stop"><span class="stop-no">${i+1}</span>${refInline(x)}</span>`).join('<span class="route-arrow">→</span>')}</div></article>`).join('')}</section>`:''}
    <section class="section"><h2>Spot sequence</h2><div class="route-stops vertical">${(r.sequence||r.sequence_refs||[]).map((x,i)=>`<span class="route-stop"><span class="stop-no">${i+1}</span>${refInline(x)}${x.role?` <small>${esc(x.role)}</small>`:''}</span>`).join('')}</div></section>
...
function renderSpot(data) {
  ...
  app.innerHTML=`<section class="hero"><div class="kicker">Spot · ${esc(data.id)}</div><h1>${esc(data.title)}</h1><p class="summary">${esc(data.summary||'')}</p></section>
    ...
    ${(s.value_points||[]).length?`<section class="section"><h2>価値</h2><ul class="list">${s.value_points.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></section>`:''}
    ${(s.practicality||[]).length?`<section class="section"><h2>実用情報</h2><ul class="list">${s.practicality.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></section>`:''}
    ${(s.related||[]).length?`<section class="section"><h2>関連</h2><div class="cards">${s.related.map(refCard).join('')}</div></section>`:''}`;
}

function initRouteMap(mapSpec,sequence){const spots=Object.fromEntries(sequence.map((x,index)=>[x.id,{...x,sequenceOrder:index+1}]));const conceptual=document.querySelector('#map-conceptual'),actual=document.querySelector('#map-actual'),summary=document.querySelector('#route-summary');const setMode=(mode)=>{conceptual?.classList.toggle('active',mode==='conceptual');actual?.classList.toggle('active',mode==='actual');if(summary)summary.hidden=mode!=='actual';const note=document.querySelector('#map-mode-note');if(note)note.textContent=mode==='actual'?'Google Routes APIで事前計算した実道路プレビュー。閲覧時には再計算しません。':(mapSpec.note||'計画上の地点配置。');loadGoogleMap(mapSpec,spots,mode);};conceptual?.addEventListener('click',()=>setMode('conceptual'));actual?.addEventListener('click',()=>setMode('actual'));setMode('conceptual');}
